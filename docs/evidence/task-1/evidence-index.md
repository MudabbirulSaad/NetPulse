...
| Clean-machine and deployment checks (Task 1.3) | The clean Windows deployment script and CI gate cover install, restart, upgrade, uninstall, and data retention. The first workflow result still needs to pass and be linked. | [2026-08-16-deployment-and-restart.md](task-1.3/2026-08-16-deployment-and-restart.md) | in progress |
...
1. Run the updated Windows workflow and attach its clean-deployment evidence link to Task 1.3.
2. Keep a separate VM or Windows Sandbox screenshot only if the rubric specifically requires visible clean-machine proof.
3. Record the WinGet maintainer decision when pull request 418335 changes state.
4. Export the final private submission report with student details and Canvas-only screenshots kept outside the public repository.
